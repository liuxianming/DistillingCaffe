...
void TripletLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    Dtype mdist = std::max(
        margin + dist_sq_ap_.cpu_data()[i] - dist_sq_an_.cpu_data()[i],
        Dtype(0.0));
    // if there is a weight
    // if bottom.size() == 4, there is weight blob
    Dtype s_weight_ = (bottom.size() == 4) ?
        bottom[3]->cpu_data()[i] : 1.0;
...
template <typename Dtype>
void TripletLossLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
