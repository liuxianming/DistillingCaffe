...
TYPED_TEST(TripletLossLayerTest, TestForward) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    loss += this->blob_bottom_y_->cpu_data()[i] *
        std::max(Dtype(0.0), margin+dist_sq_ij-dist_sq_ik);
  }
...
/*
 * Test the triplet loss layer without weights, and bottom size = 3
 */
template <typename TypeParam>
class TripletLossLayerNoSamplingTest: public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  TripletLossLayerNoSamplingTest()
      : blob_bottom_data_i_(new Blob<Dtype>(512, 2, 1, 1)),
        blob_bottom_data_j_(new Blob<Dtype>(512, 2, 1, 1)),
        blob_bottom_data_k_(new Blob<Dtype>(512, 2, 1, 1)),
        blob_top_loss_(new Blob<Dtype>()) {
  ...
  virtual ~TripletLossLayerNoSamplingTest() {
    delete blob_bottom_data_i_;
    delete blob_bottom_data_j_;
    delete blob_bottom_data_k_;
    delete blob_top_loss_;
...
TYPED_TEST_CASE(TripletLossLayerNoSamplingTest, TestDtypesAndDevices);


TYPED_TEST(TripletLossLayerNoSamplingTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  TripletLossLayer<Dtype>layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(TripletLossLayerNoSamplingTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  TripletLossLayer<Dtype>layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
